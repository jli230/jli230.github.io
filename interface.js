...
$(document).on('keydown', function(ev) {
  ...
  if (ev.key > 0 && ev.key<5 ) {
    setcurnumber(ev.key);
    $(document).trigger('log', ['setting current number', {
      curnumber: ev.key,
    ...
    if (conflicts.length == 0) {
      ...
      showpopup('#errors');
      $(document).trigger('log', ['sending error for board', {
        elapsed: state.elapsed
      }]);
    }
  } else if (ev.key == "c") {
    ...
    commitstate(cleared);
    $(document).trigger('log', ['clearing board', {
      elapsed: state.elapsed,
